...
(function () {
  ...
  // Beendetes Turnier, das zum Bearbeiten wieder geöffnet wurde: Würfe und Strafen beendeter Spiele sind änderbar
  function editing() { return state.tournament.status === 'completed' && !!state.tournament.editing; }
  // Nach Änderungen an einem beendeten Spiel Treffer/Würfe/Quote im Ergebnis nachziehen (Sieger bleibt)
  function saveMatch(m) {
    if (m.status === 'completed' && m.result) { m.result = F.buildResult(m, m.result.winnerTeamId); m.updatedAt = F.now(); }
  ...
  function viewDashboard() {
    ...
    var html = '<section class="card hero"><div><h1>' + esc(t.name) + '</h1>' +
      ...
      '<div class="phase">' + esc(editing() ? 'Turnier wird bearbeitet' : F.phaseLabel(state)) + '</div>';
    if (t.status === 'completed') html += editing()
      ? '<button data-action="close-edit" class="primary">Bearbeitung abschließen</button>'
      : '<button data-action="open-edit" class="secondary">Turnier zum Bearbeiten öffnen</button>';
  ...
  function viewMatch(id) {
    ...
    return head + matchSummary(m) + (editing() ? throwTable(m) : '');
  }
  ...
  function matchSummary(m) {
    ...
    return '<section class="card"><div class="grid2 panels">' + block(m.homeTeamId, r.homeTeamHits, r.homeTeamThrows, r.homeTeamQuote) +
      ...
      '<h2>Verwarnungen &amp; Strafen</h2>' + penaltyList(m, editing()) +
      '<div class="actions"><button class="secondary" data-action="next-match">Nächstes Spiel</button>' +
      (F.canReopen(state, m) && (state.tournament.status !== 'completed' || editing()) ? '<button class="secondary" data-action="reopen" data-id="' + m.id + '">Ergebnis korrigieren</button>' : '') +
      '</div></section>';
  }
  ...
  var actions = {
    ...
    'reopen': function (d) {
      ...
      var m = F.matchById(state, d.id);
      delete state.tournament.editing;
      F.reopen(state, m);
      Store.save(state); render();
    },
    'open-edit': function () {
      if (!confirm('Turnier zum Bearbeiten öffnen? Würfe und Strafen der beendeten Spiele können dann korrigiert werden.')) return;
      state.tournament.editing = true;
      Store.save(state); render();
    },
    'close-edit': function () {
      delete state.tournament.editing;
